...
export function LexicalInput({
  ...
  const actionButtons: ActionButton[] = [
    {
      title: 'Send',
      icon: loading ? Loader2 : ArrowUp, // Show loading spinner when loading
          ...
          {uploadedFiles.map((file, index) => (
            ...
            >
              ...
              <Button
                ...
                onClick={() => handleRemoveFile(file)}
                aria-label={`Remove ${file.name}`}
                disabled={loading} // Disable file removal when loading
              >
                <X className='size-4' />
              </Button>
            </div>
          ...
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                ...
                className={cn(
                  'relative py-2 bg-accent dark:bg-sidebar outline-0 max-h-[200px] overflow-auto text-sm border-0',
                  loading && 'opacity-60', // Dim editor when loading
                  contentEditableClassName
                )}
                onKeyDown={handleKeyDown}
              />
                ...
                <Tooltip key={index.toString()}>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={btn.onClick || (() => handleSend(input))}
                      disabled={
                        loading || // Disable config buttons when loading
                        (btn.title === 'Send' && isSendDisabled)
                      }
                      aria-label={btn.title}
                    >
                      <btn.icon className='size-4' />
                  ...
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        loading && 'animate-spin' // Add spin animation when loading
                      )}
                      onClick={btn.onClick || (() => handleSend(input))}
